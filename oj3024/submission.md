...
```text
Input: Get total score and highest score
output: calculate if highest score and lowest score are 2 apart
constraints: I dont know what lowest score is that what kinda hard
```
...
```text
Step 1. Get input
Step 2. If to calculate only when total score is not above 30 because it impossible to have that
Step 3. see if total score - max >= max. because if it >= second highest score will = max but if not it will be lower
Step 4. print output depend on if max and min are 2 int number apart
```
