...
```text
Input: Month and Day
Output: Season that month and day from input in
Constraints: Have to add if when day >= 21 because the season will change
```
...
```text
step 1. Get input
step 2. Create if else to see what season is the input in
step 3.print output
```
...
```text
I use in range in if else to sort month first, If month is in the middle that can not change for example like month 2 that only can be winter no matter what day is it. and then create if else in situation that day >= 21 in month that % 3 == 0.
```
...
```text
Because it on exactly 21 when month is % 3 == 0
```
...
```text
Because it have to be exactly winter
```
...
```text
Because it month that % 3 == 0 but day not over 21
```
