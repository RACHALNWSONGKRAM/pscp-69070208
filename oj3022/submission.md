...
```text
Input: -Get float tempature in C F K R
-Telling what input is C F K or R
-Telling which tempature you wanna return
Output: Tempature in that unit
```
...
```text
Step 1. Get input to see which unit i have to transfer to other unit
Step 2. If else to calculate
Step 3. Print output
```
...
```text
I have to turn every unit I got no many  i got into celsius to make it easier to calculate and change to other unit
```
...
```text
Because it the same unit and it should return same input
```
...
```text
Because I use Celsius to calculate but this one doesn't contain celsius in it so it have high percent that it gonna fail
```
...
```text
Because it contain celsius in it
```
