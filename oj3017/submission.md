...
```text
Input: ใส่ราคาตอนแรกที่ยังไม่คำนวณ Vat, Service Charge
Output: ราคาตอนแรกที่ + Vat + Service Charge เข้าไปแล้ว
Constraints: ต้องสร้างเงื่อนไขให้เวลา Service Charge มีค่าไม่เกิน 50 ต้องปัดให้เป็น 50
```
...
```text
1. Get input
2. Calculate total when price // 10 <= 50. have to make it 50
3.Calculate total when price // 10 > 50.
4.Calculate total when price // 10 >= 1000. make it 1000
5.print depend on situation.
```
...
```text
Because service charge is 1000, Wanna test if my code is correct.
```
...
```text
Because service charge is under 50, have to make it 50.
```
...
```text
Because it normal situation when service charge is over 50 and under 1000
```
